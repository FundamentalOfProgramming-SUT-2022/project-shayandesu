...
void print_file_layout(char* mode, char* filename, int saved)
{
    int lenm = strlen(mode), lenf = strlen(filename);
    ...
    attron(COLOR_PAIR(1));
    for(int i = 6+lenm+2+lenf+2+1; i <= 169; i++){
        move(45, i);
        printw(" ");
    }
    attroff(COLOR_PAIR(1));
...
void openfile(char* address)
{
  ...
  //  print_line_numbers();
    ...
    char* buffer = (char*)calloc(7000, sizeof(char)), c;
    c = fgetc(fp);
    long int i = 0;
    while(c != EOF){
        buffer[i] = c;
...
/*void gui()
{
    ...
    print_line_numbers();
    //move(42, 0);
    //printw("No Files Opened.");
    print_file_layout("Normal", "<none>", 0);
    move(46, 2);
    char *command = (char*)calloc(200, sizeof(char)), *address = (char*)calloc(200, sizeof(char));
    while(1){
        getstr(command);
        move(50, 1);
        printw("%s", command);
